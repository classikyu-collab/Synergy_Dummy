...
import { STATUS_COLORS, classifyStatus, todayStr, toDateStr } from '../lib/statusColors'
...
export default function ParentCoachingList() {
  ...
  return <ParentCoachingListContent pin={verifiedPin} />
}

function ParentCoachingListContent({ pin }) {
  const { studentId } = useParams()
  ...
  const range = searchParams.get('range') === '4w' ? '4w' : 'recent'

  useEffect(() => {
    let cancelled = false
    async function load() {
      const [{ data: profileRows, error: profileErr }, { data: itemRows, error: itemErr }] = await Promise.all([
        supabase.rpc('get_parent_profile', { p_student_id: studentId, p_pin: pin }),
        supabase.rpc('get_parent_coaching_items', { p_student_id: studentId, p_pin: pin }),
      ])
      if (cancelled) return
  ...
  const rangeDays = range === '4w' ? 28 : 7
  const cutoff = toDateStr(new Date(Date.now() - rangeDays * 86400000))
  const rangeLabel = range === '4w' ? '지난 4주 기록' : '최근 7일 기록'

  const recentItems = items.filter((it) => it.date !== today && it.date >= cutoff)
  const recentByDate = []
  const dateIndex = {}
  ...
  return (
    <div style={{ position: 'relative', contain: 'layout', width: '100%', maxWidth: 520, margin: '0 auto', background: THEME.bg, fontFamily: "'Noto Sans KR', -apple-system, sans-serif", color: THEME.ink, minHeight: '100vh' }}>
      ...
      >
        ...
        <p style={{ fontSize: 17, fontWeight: 700, margin: '38px 46px 4px 46px', lineHeight: 1.4 }}>코칭 기록 보기</p>
      ...
      <div style={{ padding: '18px 16px 28px' }}>
        ...
        {recentByDate.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }}>
            <p style={{ fontSize: 14, fontWeight: 800, color: THEME.inkMuted, margin: '0 0 0' }}>{rangeLabel}</p>
            {recentByDate.map((block) => (
              <div key={block.date} style={{ background: '#fff', borderRadius: 16, padding: '12px 14px', boxShadow: '0 4px 14px -10px rgba(43,38,33,0.16)' }}>
      ...
      <ParentMenuDrawer open={open} onClose={closeMenu} studentId={studentId} active={range === '4w' ? 'coaching-4w' : 'coaching-recent'} theme={THEME} />
    </div>
  )
}
